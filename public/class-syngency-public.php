			...
			switch ( wp_remote_retrieve_response_code($response) ) {
				case 200:
					...
					foreach ( $models as $model ) {
						$model->url = get_site_url() . '/divisions/' . $atts['division'] . '/portfolios/' . basename($model->url);
						
						
					}
					$output = $this->render_template('division',array('request_url' => $request_url, 'options' => $this->options, 'models' => $models));
				break;
				case 401:
				case 403:
		...
		if ( is_wp_error( $response ) ) {
			...
			switch ( wp_remote_retrieve_response_code($response) ) {
				case 200:
					...
					if ( isset($model->galleries) ) {
						foreach ( $model->galleries as $gallery ) {
							foreach ( $gallery->files as $file ) {
								$this->console_log($file->is_image);
								if($file->is_image){
					...
					$output = $this->render_template('model',array('request_url' => $request_url, 'options' => $this->options, 'model' => $model));
				break;
				case 401:
				case 403:
					$output = '<pre>Invalid API Key</pre>';
